"""utils for flow past cylinder (PyTorch)"""
import collections
import os
import time
import io
...
def init_video(path, epochs):
    ...
    video = cv2.VideoWriter(os.path.join(path, f"FlowField_{epochs}.avi"), fourcc, fps, frame_size)
    return video

def visual(model, epochs, input_data, label, device, path="./videos"):
    model.eval()
    with torch.no_grad():
        predict = model(torch.tensor(input_data, dtype=torch.float32).to(device)).cpu().numpy()
    ...
    for t in t_set:
        u_label, v_label, p_label = label[t, :, :, 0], label[t, :, :, 1], label[t, :, :, 2]
        u_predict, v_predict, p_predict = predict[t, :, :, 0], predict[t, :, :, 1], predict[t, :, :, 2]
    ...
    l2_error_total = np.sqrt(np.sum(np.square(error[:, :, :]), axis=(1, 2))) / np.sqrt(np.sum(np.square(label[:, :, :]), axis=(1, 2)))
...
def _calculate_error(label, prediction):
    ...
    return CylinderFlowError(l2_error, l2_error_u, l2_error_v, l2_error_p)

def _get_prediction(model, inputs, label_shape, model_params, batch_size, device):
...
def calculate_l2_error(model, inputs, label, model_params, batch_size, device):
    label_shape = label.shape
    prediction = _get_prediction(model, inputs, label_shape, model_params, batch_size, device)
    output_size = model_params["out_channels"]
    label = label.reshape((-1, output_size))
    l2_errors = _calculate_error(label, prediction)
    print_log('================================Start Evaluation================================')
    print_log(f"l2_error, U: {l2_errors.l2_error_u}, V: {l2_errors.l2_error_v}, P: {l2_errors.l2_error_p}, total error: {l2_errors.l2_error}")
